...
// ═══════════════════════════════════════════
// CARGA PRODUCTOS — lee JSONBin sin API Key (lectura pública)
// ═══════════════════════════════════════════
...
function renderProducts() {
  ...
  if (!list.length) {
    g.innerHTML = '<p style="color:#aaa;grid-column:1/-1;padding:2rem 0">No se encontraron productos.</p>';
    return;
  }
...
function renderCard(p) {
  ...
  return '<div class="card" onclick="openDetail(' + p.id + ')">'
    ...
    + (p.priceMax
        ? '<p class="card-price-range">' + fmt(p.price) + ' – ' + fmt(p.priceMax) + '</p>'
...
function sendCart() {
  ...
  cart.forEach(i => {
    msg += '%E2%80%A2 ' + encodeURIComponent(i.name)
      + (i.sizeLabel ? ' (' + encodeURIComponent(i.sizeLabel) + ')' : '')
      + ' x' + i.qty + ' = ' + encodeURIComponent(fmt(i.price * i.qty)) + '%0A';
  });
...
function openDetail(id) {
  ...
  if (p.sizes && p.sizes.length > 1) {
    ds.innerHTML = '<span class="d-label">Presentacion</span><div class="size-opts">'
      + p.sizes.map((s, i) =>
        '<button class="size-opt' + (i === 0 ? ' sel' : '') + '" onclick="selectSize(this,\'' + s.label + '\',' + s.price + ')">' + s.label + '</button>'
      ).join('') + '</div>';
...
function waFromDetail() {
  ...
  const msg = '%C2%A1Hola! Me interesa este producto de Surticapilar:%0A%0A%E2%80%A2 *'
    + encodeURIComponent(p.name) + '*'
    + (selectedSize ? ' (' + encodeURIComponent(selectedSize) + ')' : '')
    + '%0AUnidades: ' + detailQty
    + '%0ATotal estimado: ' + encodeURIComponent(fmt(total))
